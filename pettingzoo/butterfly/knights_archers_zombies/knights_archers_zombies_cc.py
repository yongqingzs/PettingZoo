...
from pettingzoo.butterfly.knights_archers_zombies.src.players import Archer, Knight, Player
...
class raw_env(AECEnv, EzPickle):
    """
    atr:
    1. obs_space: 会受到sequence_space、vector_state的影响
    2. state_space: 和屏幕大小宽度直接相关
    3. agent_list: 存放实际的Agent
    4. agents: 存放agent.name
    """

    agent_list: list[Player] 
    ...
    def __init__(
        ...
        self.sequence_space = sequence_space  # 会将dead_agents从obs中删除(实验性功能)
        ...
        # Game Constants
        self._seed()
        self.spawn_rate = spawn_rate  # zombie生成速度
        self.max_cycles = max_cycles
        self.pad_observation = pad_observation  # 如果agent靠近边缘，会使state变黑
        self.killable_knights = killable_knights  # 是否会dead
        self.killable_archers = killable_archers
        self.line_death = line_death  # 靠近变线是否会dead
        self.num_archers = num_archers
        self.num_knights = num_knights
        self.max_zombies = max_zombies
        self.max_arrows = max_arrows  # 最大箭头数

        # Represents agents to remove at end of cycle
        self.kill_list = []
        self.agent_list = []
        self.agents = []  # 存放的是agent.name
        self.dead_agents = []

        # 为每个agent添加对应的名字和查找序列
    ...
    def zombit_hit_knight(self):
        """
        计算被zombie杀死的knight，在archer_list中移除knight，并且在kill_list中添加
        """
    ...
    def zombie_hit_archer(self):
        """
        计算被zombie杀死的archer，在archer_list中移除archer，并且在kill_list中添加
        """
        for zombie in self.zombie_list:
    ...
    def step(self, action):
        """
        步骤:
        1. 在每一循环的每一回合前，将迭代奖励和agent.score清空
                |
        2. 进行agents循环，每循环到一个agent，对该agent进行一次状态更新
                |
        3. 每一回合，判断agent是否出现超界done
        - 将超界的agent添加至kill_list
                |
        4. 计算武器(必要时启动武器)
                |
        5. _agent_selector.is_last:
        - 更新武器区
        - Zombie Kills the Sword/Arrow/Archer/Knight
        - update some zombies
        - 生成一批zombie
        - 判断游戏结束(防线被突破，agents均dead) -> 返回self.run
        - 回合数 + 1
                |
        6. 在_agent_selector.is_last外部:
        - 根据self.run判断terminate
        - 根据最大循环数判断truncate
                |
        7. _agent_selector.is_last:
        - 根据_live_agents(从self.agents复制，通过kill_list remove)重新生成_agent_selector
                |
        8. agent_selection = self._agent_selector.next()
        - 将next_agent.score给rewards
                |
        9. 收尾
        - _accumulate_rewards()
        - _deads_step_first()
        ...
        self._cumulative_rewards[self.agent_selection] = 0  # 每一回合的迭代奖励
        agent.score = 0  # 将score存在每一Agent中

        # this is... so whacky... but all actions here are index with 1 so... ok
        action = action + 1  # 解决action index问题
        out_of_bounds = agent.update(action)  # agent运动，并且返回(是否超出边界)

        # check for out of bounds death
        if self.line_death and out_of_bounds:  # 只有line_death时，out_of_bounds才会生效
            agent.alive = False
        ...
        if self._agent_selector.is_last():
            ...
            # update some zombies
            for zombie in self.zombie_list:
                zombie.update()  # zombie在每一回合结束时进行更新

            # Spawning Zombies at Random Location at every 100 iterations
            self.spawn_zombie()  # 生成一批zombie
        ...
        self.rewards[self.agent_selection] = next_agent.score  # 添加进rewards中的是next_agents
